...
| Date | What | Result |
|---|---|---|
| 2026-09-06 | Runtime refreshed from `2a1f480` (fixed tags `0.27.1-rocm7.2.3-gfx1030` + `latest`, digest `sha256:5d2a9cb0…`; base unchanged) | **PASS (smoke).** Functionally identical to the 2026-08-31 image — presets never pinned `NCCL_P2P_LEVEL`, entrypoint changes were log text — so no serve test: revision label = main, `list-presets`, `DRYRUN=1 preset:qwen38-27b-gptq` (both plugins, lookup-only TunableOp), plugin import, the rewritten TROUBLESHOOTING.md baked in. Old digest stays on GHCR untagged. |
