...
| `VLLM_USE_V2_MODEL_RUNNER` | **unset** (V1) | — | Do not copy from the 122B configuration: V2 is required only for MTP under pipeline parallelism (patch 0009). On everything else it is a reported source of large-prompt failures — `=0` "fixed all my problems" (CorbinD, gfx1030 club Discord; TROUBLESHOOTING.md §1.1). |
| `NCCL_P2P_LEVEL` | **unset** (RCCL autodetects) | topology-dependent | Do not copy from another host. `PXB` disabled peer access outright on a one-V620-per-switch host (`rccl-tests` 8.0 vs 21.0 GB/s — Waldecir Santos, [#2](https://github.com/leapdragon/vllm-rdna2-recipe/pull/2)); `SYS` gave +8 % prefill on the 2+2 development host. Check `rocm-smi --showtopo` before pinning (TROUBLESHOOTING.md §1.5). Was pinned to `PXB` in the stability stack below until 2026-09-06. |
...
| `EXTRA_ENV` | `VLLM_USE_V2_MODEL_RUNNER=1` (122B under PP only; elsewhere leave unset or `0` — TROUBLESHOOTING.md §1.1) | **MTP under PP requires the V2 model runner** — the V1 drafter path page-faults under PP (patch 0009's section in 01-PATCHES). Comma-separated `NAME=VALUE` passthrough. |
...
| env | ~~`NCCL_P2P_LEVEL=PXB`~~ (removed 2026-09-06) | Was part of the measured stack; topology-dependent and now left unset — see the runtime table above and TROUBLESHOOTING.md §1.5. The stability result was measured with it set on the 2+2 host. |
...
([TROUBLESHOOTING §2.1](TROUBLESHOOTING.md)) and it costs nothing.
